...
const submission = ({ inputDataSet, submittedDataSet, outputToFile }) => {
  ...
  function parseInputDataSet(data) {
    ...
    for (let i = 0; i < numProjects; i++) {
      const [name, numDays, score, bestBeforeDay, numRoles] = parseLine(
        lines[currentLine++],
        [1, 2, 3, 4]
      );
  ...
  function parseSubmittedDataSet(dataset, submittedData) {
    ...
    for (let i = 0; i < numExecutedProjects; i++) {
      ...
      if (typeof project.receivedScore !== "undefined") {
        throw createError(
          `Project ${projectName} is listed twice`,
          currentLine
        );
      }
      ...
      const duplicatedNames = contributorNames.filter(
        (name, idx, array) => array.indexOf(name) !== idx
      );
      ...
      const nonExistentContributor = contributorNames.find(
        (name) => !dataset.contributors[name]
      );

      if (nonExistentContributor) {
        throw createError(
          `Contributor "${nonExistentContributor}" does not exist`,
          currentLine
        );
      }
      ...
      const contributorsAreAvailableFrom = Math.max(
        ...contributorNames.map(
      ...
      for (let j = 0; j < numContributors; j++) {
        ...
        const mentored =
          (contributor.skills[skill] || 0) === level - 1 &&
          contributorNames.some(
            (contributorName) =>
        ...
        if (contributor.skills[skill] >= level || mentored) {
          ...
          contributor.waitingDays +=
            contributorsAreAvailableFrom - contributor.availableFrom;
          contributor.availableFrom = endedOn;
          continue;
        }

        const highestSkillLevel = Math.max(
          ...contributorNames.map(
            (contributorName) =>
              dataset.contributors[contributorName].skills[skill] || 0
          )
        );
        ...
        throw createError(
          [
            ...
            `${name} has the ${skill} skill at level ${
              contributor.skills[skill] || 0
            },`,
    ...
    if (
      typeof lines[currentLine] !== "undefined" &&
      lines[currentLine].trim().length
    ) {
      throw createError(
        `Expected end of file, got: ${lines[currentLine]}`,
        currentLine + 1
      );
    }
  ...
  function createInsights(dataset) {
    ...
    const toPercentage = (value) =>
      yellow(`${(value * 100).toString().match(/^-?([\d][^.]*)/)[0]}%`);
    ...
    const {
      ...
      contributors,
      projects,
      numTimesContributorsGotMentored,
      numTimesContributorsIncreasedSkillLevel,
      numContributorsWorkedOnProjects,
    } = dataset;
    const projectNames = Object.keys(projects);
    const contributorNames = Object.keys(contributors);
    const completedProjects = projectNames.filter(
      (name) => typeof projects[name].receivedScore !== "undefined"
    );
    const numProjectsCompleted = completedProjects.length;
    const { score, numProjectsBestBeforeDay, numProjectsCompletedTooLong } =
      completedProjects.reduce(
        (sum, name) => {
          const project = projects[name];

          sum.score += project.receivedScore;
          sum.numProjectsBestBeforeDay +=
            project.receivedScore === project.score;
        ...
        {
          score: 0,
          numProjectsBestBeforeDay: 0,
          numProjectsCompletedTooLong: 0,
        }
      );
    const averageWaitingDays = (
      contributorNames.reduce(
        (sum, name) => sum + contributors[name].waitingDays,
        0
      ) / numContributorsWorkedOnProjects || 0
    ).toFixed(2);
    const numContributorsWorked = contributorNames.filter(
      (name) => contributors[name].availableFrom > 0
    ).length;
        ...
        [
          ...
          [
            [
              ...
              `(${toPercentage(
                numProjectsCompleted / numProjects
              )} of all projects)`,
            ].join(" "),
            [
              '- projects completed before their "best before" time in days',
              `(scoring full points): ${formatNumber(
                numProjectsBestBeforeDay
              )}`,
              `(${toPercentage(
                numProjectsBestBeforeDay / numProjects
              )} of all projects)`,
            ].join(" "),
            [
              ...
              `(${toPercentage(
                numProjectsCompletedTooLong / numProjects
              )} of all projects)`,
            ].join(" "),
            [
            ...
            [
              ...
              `completing a project: ${formatNumber(
                numTimesContributorsIncreasedSkillLevel
              )}`,
            ].join(" "),
            [
